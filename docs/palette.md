...
| Piece | Status |
|---|---|
| `tools/quantize.gd` — maps an imported PNG onto the palette | Phase 3b |
| `tools/check_palette.sh` — fails on any off-palette pixel | Phase 3b, CI gate 6 |

Until those exist, the rule is honoured by hand.

### Exemption: the Phase 3a debug swatches

`assets/tiles/grass.png`, `assets/tiles/water.png` and
`assets/objects/oak_tree.png` are deliberately **off-palette**. They are flat
